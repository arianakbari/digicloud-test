...
class FeedItemRepository(FeedItemRepositoryAbstract):
    ...
    def toggle_favorite(self, id: int, user_id: int):
        UserFeedItem.objects.filter(user=user_id, feed_item=id).update(
    ...
    def get_comments(self, feed_item_id, user_id: int):
        return FeedItemComment.objects.filter(
            feed_item_id=feed_item_id, feed_item__feed__user=user_id
        )
